...
// Domain - BANK
// =============================================
function get_rekening(): array { return read_json(FILE_REKENING); }
function find_rekening(string $no_rek): ?array {
    foreach (get_rekening() as $r) if ($r['no_rek'] === $no_rek) return $r;
    return null;
}
...
function mutasi_per_hari(int $hari = 7): array {
    $mut = read_json(FILE_MUTASI);
    $out = [];
    for ($i = $hari-1; $i >= 0; $i--) {
...
// =============================================
function get_kartu(): array { return read_json(FILE_KARTU); }
