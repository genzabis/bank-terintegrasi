...
$peers = ping_peers();
$audit = get_audit(80);
$stat = ['AppsBank'=>['in'=>0,'out'=>0,'fail'=>0],'AppsPendidikan'=>['in'=>0,'out'=>0,'fail'=>0],'AppsTravel'=>['in'=>0,'out'=>0,'fail'=>0]];
foreach ($audit as $a) {
...
layout_start('Monitor Integrasi', 'Status real-time dan log komunikasi REST dengan 3 sistem peer');
?>
  ...
  <?php foreach ($peers as $name=>$p): ?>
    <div class="card">
      ...
      <div class="row cols-3 mt">
        ...
        <div><div class="muted small">Outgoing</div><div class="bold"><?= $stat[$name]['out']??0 ?></div></div>
...
<div class="card no-pad">
  ...
  <div class="scroll" style="max-height:540px">
    <ul class="log-list">
      ...
      <?php endforeach; ?>
      <?php if (!$audit): ?><div class="no-data">Belum ada aktivitas integrasi. Lakukan checkout / bundle untuk mulai mengisi log.</div><?php endif; ?>
    </ul>
  </div>
</div>
